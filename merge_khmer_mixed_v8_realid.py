...
BASE_DIR = Path("data/khmer_mixed_v7")
REAL_ID_DIR = Path("data/khmer_real_id_style_v1")

OUT_DIR = Path("data/khmer_mixed_v8_realid")
...
def main():
    ...
    copy_and_collect(read_labels(REAL_ID_DIR / "train_label.txt"), REAL_ID_DIR, train_rows)
    copy_and_collect(read_labels(REAL_ID_DIR / "val_label.txt"), REAL_ID_DIR, val_rows)

    save_labels(train_rows, OUT_DIR / "train_label.txt")
    save_labels(val_rows, OUT_DIR / "val_label.txt")

    print("Mixed v8 real-ID dataset created successfully.")
