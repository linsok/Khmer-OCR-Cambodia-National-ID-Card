...
BASE_DIR = Path("data/khmer_mixed_v3")
ID_STRUCT_DIR = Path("data/khmer_id_focus_v2")

OUT_DIR = Path("data/khmer_mixed_v4")
...
def main():
    ...
    id_train = read_labels(ID_STRUCT_DIR / "train_label.txt")
    id_val = read_labels(ID_STRUCT_DIR / "val_label.txt")
    ...
    copy_and_collect(id_train, ID_STRUCT_DIR, train_rows)
    copy_and_collect(id_val, ID_STRUCT_DIR, val_rows)

    save_labels(train_rows, OUT_DIR / "train_label.txt")
    save_labels(val_rows, OUT_DIR / "val_label.txt")

    print("Mixed v4 dataset created successfully.")
